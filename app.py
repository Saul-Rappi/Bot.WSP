...
def crear_app():
    ...
    def contar_pendientes_chat():
        try:
            wb = load_workbook(EXCEL_FILE)
            sheet = wb['ChatEntrante']
            # Contar filas que tienen datos y están pendientes (excluyendo el encabezado)
            count = sum(1 for row in sheet.iter_rows(min_row=2) 
                       if row[0].value is not None and 
                          (not row[-1].value or row[-1].value.lower() == 'pendiente'))  # Asumiendo que el estado está en la última columna
            return count
        except Exception as e:
            print(f"Error al contar registros pendientes: {str(e)}")
            return 0
    ...
    def dashboard():
        ...
        # Obtener el conteo de pendientes de ChatEntrante
        pendientes_mini = contar_pendientes_chat()
        ...
        return render_template('dashboard.html',
                              ...
                              enviados=enviados,
                              pendientes_mini=pendientes_mini,  # Usar el conteo de ChatEntrante
                              llamadas=llamadas,
                              chats=chats,
                              agendados=agendados,
